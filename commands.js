...
function getAllIds(plid, token, idList, cb) {
	...
	youtube.playlistItems.list({
		...
		if (results.nextPageToken) return getAllIds(plid, results.nextPageToken, idList, cb);
		cb(null, idList);
	});
}
...
var commands = {
	...
	help: {
		...
		exe: (bot, msg, ...args) => {
			...
			} else if (commands[args[1]] !== undefined) {
					...
					case `video`:
						...
						ytdl.getInfo(args[1], (err, info) => {
							if (err) {
								...
								send(msg.channel, `Error adding video: ${err}`, {
									code: true,
								}, 20000);
							...
							if (start > 0) {
								...
								getAllIds(plId, (err, results) => {
									...
									results.forEach(element => {
										// Get the video metadata
										ytdl.getInfo(`www.youtube.com/watch?v=${element}`, (e, info) => {
											if (e) {
					...
					case `search`:
						{
							...
							youtube.search.list({
								...
								ytdl.getInfo(`www.youtube.com/watch?v=${results.items[0].id.videoId}`, (e, info) => {
									if (e) {
			...
			if (args.length > 2) {
				...
				fs.readFile(`tags.json`, (err, data) => {
					...
					if (tags[msg.channel.guild.id][tagname] === undefined) {
						...
						fs.writeFile(`tags.json`, JSON.stringify(tags), (e) => {
							if (e) {
								console.log(e);

...
// the object name is the actual command to be entered - this must be all lowercase
				...
				fs.readFile(`tags.json`, (err, data) => {
					...
					try {
						...
						if (tags[msg.channel.guild.id] === undefined || tags[msg.channel.guild.id].length === 0) {
							lib.send(msg.channel, `There are no tags on this server`, 8000);
						} else {
				...
				fs.readFile(`tags.json`, (err, data) => {
					...
					try {
						...
						if (tags[msg.channel.guild.id][tagname] === undefined) {
							...
							fs.writeFile(`tags.json`, JSON.stringify(tags), (er) => {
								if (er) {
									lib.log(`error`, `${err}`);
